...
pub fn map_render_land(
	gd: &mut GameData, center_x: usize, center_y: usize,
) {
		...
		for j in (0..gd.map_render_size * 2).rev() {
				...
				&& ((yy as usize) < gd.lp.wi.map_size)
			{
				...
				let elev = gd
					.lp
					.topography
					.read(gd.lp.topography.TERRAIN, gd.index);
				//let elev_rel = (elev as f32) / 255.0;

				let river_width =
					gd.lp.rivers.read(gd.lp.rivers.WIDTH, gd.index);
				...
				//Or do it randomly
				let random =
					prng::get(0.0, 1.0, gd.lp.wi.seed, gd.index);
				...
				if ((xx as usize) == center_x)
					&& ((yy as usize) == center_y)
				{
