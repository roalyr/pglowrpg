...
pub fn write_save(
	...
	//Write the data
	write_data(lp, wg_str, options_worldgen, &save_dir);

	//Optionally render colorful images
	write_images_color(lp, wg_str, options_worldgen, &save_dir);

	//Optionally render raw images
	if options_worldgen.render_raw_maps {
		write_images_raw(lp, wg_str, options_worldgen, &save_dir);
	}
}
