...
pub fn with_water(
	...
	let random = prng::get(0.0, 1.0, lp.wi.seed, rg.dv.x0 * rg.dv.y0 * iter);
...
fn seek_waterbodies(
	...
	loop {
		for (i, j) in
			BresenhamCircle::new(rg.dv.x0 as i32, rg.dv.y0 as i32, rg.dv.r as i32)
		{
			let i = i as usize;
			let j = j as usize;
			...
			if (i < size) && (j < size) {
				...
				if (wmask >= lp.wi.river_attr_pool_size_pow) && (temp_abs > TEMP_POLAR)
				{
					rg.dv.x1 = i;
					rg.dv.y1 = j;
					rg.dv.hit = true;
...
fn project_randomly(
	...
	let mut shift = 2.0
		* std::f32::consts::PI
		* noise_maps::point_multi(noise_factor, lp.wi.seed, rg.dv.x0, rg.dv.y0);
