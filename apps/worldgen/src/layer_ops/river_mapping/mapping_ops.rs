...
fn map_rivers_reverse(
  ...
  let index_river_source = lp.xy.ind(rg.river_source.0, rg.river_source.1);
  ...
  if river_length >= lp.wi.river_min_length {
    ...
    for n in path_array.windows(2) {
      ...
      match river_elem_downstr {
        NO_RIVER => {
          sort_uninterrupted(rg, lp, index_current, index_river_source);
        ...
        RIVER_BODY => {
          ...
          if terrain_downstr < terrain_current {
            lp.rivers
              .write(RIVER_WATERFALL, lp.rivers.ELEMENT, index_downstr);
          }
