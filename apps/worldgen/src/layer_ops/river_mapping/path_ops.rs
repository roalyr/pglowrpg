...
fn make_paths(
	...
	if (random <= total_prob) && (wmask == NO_WATER) {
		...
		progress::percent_step(rg.river_count_number, rg.river_est_number, 10);
		...
		let nodes = pathfinding_nodes(rg, lp, seg_len, &terrain_map, diag_flag);
		...
		for node_pair in nodes.windows(2) {
			...
			let path_array_seg = pathfinding_nodes(rg, lp, 1, &random_map, diag_flag);
...
fn pathfinding_nodes(
	...
	//iter 1
	let result_init =
		path::make(&rg.dv, &terrain_map, lp.wi.map_size, diag_flag, seg_len);
...
pub fn get_random_map(
	...
	{
		*cell_v = (array_noise1[index] * 255.0 * (1.0 - lp.wi.river_noise_blend)
			+ array_noise2[index] * 255.0 * lp.wi.river_noise_blend) as u8;
	}
	random_map
}
