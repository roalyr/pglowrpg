...
macro_rules! prompt_input {
	...
	($flag: expr;  $word_list: expr; $b: block) => {
		{
			use lib_text_ops::input::prompt_word;
			let mut input = String::new();
				...
				true => {
					while input.is_empty() {
						...
						match $flag {
							...
							_ => {
								lib_text_ops::input_flag_error_word($flag.to_string());
								panic!();
							}
						}
					}
	...
	($flag: expr; $b: block) => {
		{
			use lib_text_ops::input::prompt_option;
			use lib_text_ops::input::prompt_number;
			let mut input = String::new();
			match OPTIONS.repeat_text_if_no_input{
				// Keep pestering the player to no end
				true => {
					while input.is_empty() {
						...
						match $flag {
							...
							_ => {
								lib_text_ops::input_flag_error($flag.to_string());
								panic!();
							}
						}
					}
